...
single net at 0.883. Finally, an **Optuna HPO → diverse ensemble** pipeline
([`higgs_hpo_ensemble.py`](./higgs_hpo_ensemble.py)) reached **test AUC = 0.893 on the
21 low-level features** (and 0.890 on all 28) — the **overall best, beating the paper's
0.885 by ~0.008.** Notably the 21-feature model *beats* the 28-feature one: the 7
hand-engineered features are redundant for a deep net (they are deterministic functions
of the low-level inputs) and slightly *hurt* it by adding overfitting surface. Saved,
loadable ensembles live in `models/` (reload with `higgs_predict.py`). This document
explains every decision so you can understand it, tune it, and implement it yourself.
...
| **This plan — 8-model ensemble** | complete (28) | **0.885 (measured)** | hand-picked diverse configs |
| **This plan — HPO best single net** | low-level (21) | **0.891 (measured)** | tuned `[1024]×6`, all-layer dropout |
| **This plan — HPO + 12-model ensemble** | complete (28) | **0.890 (measured)** | tuned + ensembled |
| **This plan — HPO + 12-model ensemble** | low-level (21) | **0.893 (measured)** | **best overall — 21 feats beat 28** |
...
- **HPO + ensemble is the best of all (test 0.890).** A 60-trial Optuna search
  ([`higgs_hpo_ensemble.py`](./higgs_hpo_ensemble.py)) found **wide** nets (`[1024]×5–6`, GELU,
  all-layer dropout ~0.15, LR ~3.8e-3) that beat the hand-picked configs (single 0.885 vs 0.883),
  with **no validation-overfitting gap** (val ≈ test). Ensembling the top 6 distinct configs ×
  2 seeds reached **0.890**, then plateaued — the dataset's practical ceiling. Tuning bought the
  single model ~+0.002; the ensemble bought another ~+0.005.
- **Fewer features win: low-level (21) beats complete (28) — test 0.893 vs 0.890.** Running the
  same HPO→ensemble pipeline on only the 21 low-level features gave the **overall best** (single
  0.891, ensemble **0.893**), beating the 28-feature version at both levels — by ~+0.006 as a
  single net (0.891 vs 0.885) and ~+0.003 as an ensemble (0.893 vs 0.890; ensembling cancels some
  of the overfitting the redundant features cause, so the gap narrows). Interpretation: the 7 high-level features are *deterministic
  functions of the low-level ones*, so they carry no new information for a deep net — but they add
  redundant, correlated inputs that enlarge the overfitting surface and slightly hurt. This both
  reproduces and *sharpens* the paper's thesis: the net doesn't just match hand-engineered features,
  it does better without them. The practical ceiling for this dataset is ~0.893, on the low-level set.
